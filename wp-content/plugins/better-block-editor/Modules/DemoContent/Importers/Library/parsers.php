		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
				foreach ( $result->get_error_data() as $error ) {
					echo absint( $error->line ) . ':' . absint( $error->column ) . ' ' . esc_html( $error->message ) . "\n";
				}
			} elseif ( 'XML_parse_error' == $result->get_error_code() ) {
				$error = $result->get_error_data();
				echo absint( $error[0] ) . ':' . absint( $error[1] ) . ' ' . esc_html( $error[2] );
			}
			echo '</pre>';
			echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'better-block-editor' ) . '</strong><br />';
			echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'better-block-editor' ) . '</p>';
		}
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		/**
		 * CUSTOM edition. On some hosts 'DOMDocument' may be not available despite the 'simplexml' extension are enabled.
		 */
		if ( ! class_exists( 'DOMDocument', false ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'Cannot use DOMDocument to parse file - class does not exist', 'better-block-editor' ), libxml_get_errors() );
		}
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'better-block-editor' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'better-block-editor' ) );
		}
