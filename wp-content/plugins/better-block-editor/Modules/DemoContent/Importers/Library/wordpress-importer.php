...
if ( class_exists( 'WP_Importer' ) ) {
	...
	function import_start( $file ) {
			if ( ! is_file( $file ) ) {
				echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'better-block-editor' ) . '</strong><br />';
				echo esc_html__( 'The file does not exist, please try again.', 'better-block-editor' ) . '</p>';
				$this->footer();
				die();
			}
		...
		function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'better-block-editor' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'better-block-editor' ) . '</a>' . '</p>';
			echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'better-block-editor' ) . '</p>';
			...
			if ( ! empty( $import_data['authors'] ) ) {
				...
				foreach ( $import_data['posts'] as $post ) {
					...
					if ( empty( $login ) ) {
						...
						echo esc_html( sprintf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'better-block-editor' ), $post['post_author'] ) );
						echo esc_html( self::LOG_ERROR_SEPARATOR );
						continue;
					}
		...
		function get_author_mapping() {
			...
			foreach ( $authors as $i => $old_login ) {
		...
		function process_categories() {
			...
			foreach ( $this->categories as $cat ) {
				...
				if ( ! is_wp_error( $id ) ) {
					...
					echo esc_html( sprintf( __( 'Failed to import category %s', 'better-block-editor' ), $cat['category_nicename'] ) );
		...
		function process_tags() {
			...
			foreach ( $this->tags as $tag ) {
				...
				if ( ! is_wp_error( $id ) ) {
					...
					echo esc_html( sprintf( __( 'Failed to import post tag %s', 'better-block-editor' ), $tag['tag_name'] ) );
					echo ': ' . esc_html( $id->get_error_message() );
					echo esc_html( self::LOG_ERROR_SEPARATOR );
					continue;
				}
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! post_type_exists( $post['post_type'] ) ) {
					echo esc_html(
						sprintf(
							...
							__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'better-block-editor' ),
							$post['post_title'],
							$post['post_type']
						)
					);
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					echo esc_html( sprintf( __( '%1$s &#8220;%2$s&#8221; already exists.', 'better-block-editor' ), $post_type_object->labels->singular_name, $post['post_title'] ) );
					echo esc_html( self::LOG_ERROR_SEPARATOR );
				...
				if ( is_wp_error( $post_id ) ) {
					...
					echo esc_html(
						sprintf(
							/* translators: 1: Post type singular name, 2: Post title */
							__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'better-block-editor' ),
							$post_type_object->labels->singular_name,
							$post['post_title']
						)
					);
					echo ': ' . esc_html( implode( ', ', $post_id->get_error_messages() ) );
					echo "\n";
					continue;
				}
					...
					foreach ( $post['terms'] as $term ) {
						...
						if ( ! $term_id ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								echo esc_html( sprintf( __( 'Failed to import %1$s %2$s', 'better-block-editor' ), $taxonomy, $term['name'] ) );
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				esc_html_e( 'Menu item skipped due to missing menu slug', 'better-block-editor' );
				echo esc_html( self::LOG_ERROR_SEPARATOR );
				return;
			}
			...
			if ( ! $menu_id ) {
				...
				echo esc_html( sprintf( __( 'Menu item skipped due to invalid menu slug: %s', 'better-block-editor' ), $menu_slug ) );
				echo esc_html( self::LOG_ERROR_SEPARATOR );
				return;
			} else {
		...
		function process_attachment( $post, $url ) {
			if ( ! $this->fetch_attachments ) {
				return new WP_Error(
					...
					__( 'Fetching attachments is not enabled', 'better-block-editor' )
				);
			}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'better-block-editor' ), size_format( $max_size ) ) );
			}
		...
		function backfill_attachment_urls() {
			...
			foreach ( $this->url_remap as $from_url => $to_url ) {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->postmeta} SET meta_value = REPLACE(meta_value, %s, %s) WHERE meta_key='enclosure'", $from_url, $to_url ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			}
		}
		...
		public function get_timelimit() {
			return apply_filters( 'wpbbe_importer_timelimit', $this->timelimit );
		}
		 ...
		 */
		protected function add_logger_message( $message ) {
			echo esc_html( $message . static::LOG_MESSAGE_SEPARATOR );
		}
	}
