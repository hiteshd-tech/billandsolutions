...
namespace The7_Post_Types\Handlers;
	...
	protected function validate( $data ) {
		...
		if ( empty( $data[ self::FIELD_POST_TYPE_ARGS ]['name'] ) ) {
			the7_admin_notices()->add( 'the7_core_no_post_type_name', function() {
				echo '<p>' . esc_html__( 'Please provide a post type name', 'dt-the7-post-types' ) . '</p>';
			}, 'error' );
		...
		if ( false !== strpos( $data[ self::FIELD_POST_TYPE_ARGS ]['name'], '\'' ) ||
			...
			the7_admin_notices()->add( 'the7_core_quotes_in_post_type_name', function() {
				echo '<p>' . esc_html__( 'Please do not use quotes in post type names or rewrite slugs', 'dt-the7-post-types' ) . '</p>';
			}, 'error' );
		...
		if ( $this->maybe_new_entry( $post_type_slug ) ) {
			...
			if ( $this->slug_is_used_by_another_post_type( $post_type_slug, Post_Types::get() ) ) {
				...
				the7_admin_notices()->add( 'the7_core_slug_is_used_by_another_post_type', function () use ( $post_type_slug ) {
					printf( '<p>' . esc_html__( 'Please choose a different post type name. %s is already registered.', 'dt-the7-post-types' ) . '</p>', $post_type_slug );
				}, 'error' );
			...
			if ( $slug_is_used_by_post ) {
				...
				the7_admin_notices()->add( 'the7_core_slug_is_used_by_post', function () use ( $slug_is_used_by_post ) {
					printf( '<p>' . esc_html__( 'Please choose a different post type name. %s matches an existing page slug, which can cause conflicts.', 'dt-the7-post-types' ) . '</p>', '<a href="' . esc_url( get_permalink( $slug_is_used_by_post ) ) . '">' . esc_html( $slug_is_used_by_post->post_name ) . '</a>' );
				}, 'error' );
