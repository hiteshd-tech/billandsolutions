...
class UI {
	...
	public function get_help( $help_text = '' ) {
		return '<a href="#" class="the7-help dashicons-before dashicons-editor-help" title="' . esc_attr( $help_text ) . '"></a>';
	}
	...
	public function get_description( $help_text = '' ) {
		return '<p class="the7-field-description description">' . $help_text . '</p>';
	}
	...
	public function get_select_input( $args = [] ) {
		...
		if ( ! empty( $args['selections']['options'] ) && is_array( $args['selections']['options'] ) ) {
			foreach ( $args['selections']['options'] as $val ) {
				$result = '';
				$bool   = Utility::disp_boolean( $val['attr'] );

				if ( is_numeric( $args['selections']['selected'] ) ) {
					$selected = Utility::disp_boolean( $args['selections']['selected'] );
	...
	/**
	 * Return a switch control.
	 *
	 * @param array $args Arguments.
	 * @return string Switch control to output.
	 */
	public function get_switch( $args = [] ) {
		$defaults = $this->get_default_input_parameters(
			[
				'options' => [],
				'default' => null,
		...
		if ( is_array( $args['options'] ) ) {
			...
			foreach ( $args['options'] as $val => $label ) {
				$value .= '<option value="' . esc_attr( $val ) . '"' . selected( $val, $selected, false ) . '>' . $label . '</option>';
			}
		}
	...
	public function get_check_input( $args = [] ) {
		...
		$value .= '<input ' . implode( ' ', presscore_convert_indexed2numeric_array( '=', $props, '', '"%s"' ) ) . checked( $args['checked'], true, false ) . ' />';
	...
	public function get_menu_icon_preview( $menu_icon = '' ) {
		...
		return '<div id="the7-menu-icon-preview">' . $content . '</div>';
	}
