...
class Photo_Scroller extends The7_Elementor_Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .scroller-arrow' => 'border-color: {{VALUE}}; outline-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .scroller-arrow:hover' => 'border-color: {{VALUE}}; outline-color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
			...
			if ( $title || $description ) {
				if(! empty( $image_data['caption'] ) || ! empty( $image_data['description'] ) || ! empty($image_data['title'] ) || ! empty( $image_data['alt'] )){
					$desc_html = '<div class="album-content-description" ><div class="content-description-inner">';
							...
							if ( $has_description && isset( $settings['overlay_description'] ) ) {
		...
		<div <?php echo $this->get_render_attribute_string( 'scroller-wrapper' ); ?>>
			...
			<div class="scroller-arrow prev" role="button" tabindex="0" aria-label="Prev slide"><?php Icons_Manager::render_icon( $settings['prev_icon'] ?? [], [ 'aria-hidden' => 'true' ] ); ?></div>
			<div class="scroller-arrow next"  role="button" tabindex="0" aria-label="Next slide"><?php Icons_Manager::render_icon( $settings['next_icon'] ?? [], [ 'aria-hidden' => 'true' ] ); ?></div>
		</div>
		<?php
	}
