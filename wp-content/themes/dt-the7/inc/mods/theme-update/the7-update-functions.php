 ...
 * @return void
 */
function the7_update_12_3_0_set_theme_json_version_to_1() {
	if ( the7_is_gutenberg_theme_mode_active() ) {
		$fse_compatibility = The7\Mods\Compatibility\Gutenberg\Block_Theme\The7_Block_Theme_Compatibility::instance();
		if ( ! $fse_compatibility->fse_version_exists() ) {
			$fse_compatibility->set_fse_version( '1' );
		}
	}
}
 ...
 * @return void
 */
function the7_update_12_5_0_set_theme_json_version_to_2() {
	if ( the7_is_gutenberg_theme_mode_active() ) {
		$fse_compatibility = The7\Mods\Compatibility\Gutenberg\Block_Theme\The7_Block_Theme_Compatibility::instance();
		if ( ! $fse_compatibility->fse_version_exists() ) {
			$fse_compatibility->set_fse_version( '2' );
		}
	}
}
 ...
 * @return void
 */
function the7_update_12_7_0_set_fvm_integration_off() {
	the7_apply_theme_options_migration( new \The7\Mods\Theme_Update\Migrations\v12_7_0\Fvm_Integration_Migration() );
}

/**
 * @return bool
 */
function the7_update_12_10_0_switch_to_better_block_editor() {
	if ( ! is_plugin_active( 'the7-block-editor/the7-block-editor.php' ) ) {
		return false;
	}
	...
	$tgmpa             = The7_TGM_Plugin_Activation::get_instance();
	$plugin_to_install = 'better-block-editor';

	if ( empty( $tgmpa->plugins[ $plugin_to_install ] ) ) {
		return false;
	}

	if ( ! $tgmpa->is_plugin_installed( $plugin_to_install ) ) {
		the7_tgmpa_register_bulk_classes();
		the7_get_filesystem();
		...
		$installer = new The7_TGMPA_Bulk_Installer(
			new WP_Ajax_Upgrader_Skin(
				[
					'url'          => esc_url_raw( $tgmpa->get_tgmpa_url() ),
					'nonce'        => 'bulk-plugins',
					'names'        => [ $tgmpa->plugins[ $plugin_to_install ]['name'] ],
					'install_type' => 'install',
				]
			)
		);

		$installer->bulk_install( [ $tgmpa->get_download_url( $plugin_to_install ) ] );

		ob_end_clean();
	}

	if ( $tgmpa->can_plugin_activate( $plugin_to_install ) ) {
		activate_plugin( $tgmpa->plugins[ $plugin_to_install ]['file_path'], '', false, true );
	}
 ...
 * @return bool
 */
function the7_update_12_10_0_installing_bbe_plus() {
	Presscore_Modules_TGMPAModule::init_the7_tgmpa();
	Presscore_Modules_TGMPAModule::register_plugins_action();

	$tgmpa         = The7_TGM_Plugin_Activation::get_instance();
	$parent_plugin = 'better-block-editor';

	if ( ! $tgmpa->is_plugin_installed( $parent_plugin ) ) {
		return false;
	}

	$plugin_to_install = 'better-block-editor-pro-kit';
	...
	if ( $tgmpa->is_plugin_activated( $parent_plugin ) && $tgmpa->can_plugin_activate( $plugin_to_install ) ) {
		activate_plugin( $tgmpa->plugins[ $plugin_to_install ]['file_path'], '', false, true );
	}
 ...
 * @return bool
 */
function the7_update_12_10_0_update_theme_fse_files() {
	// Update block theme files after switching to Better Block Editor.
	The7\Mods\Compatibility\Gutenberg\Block_Theme\The7_Block_Theme_Compatibility::instance()->copy_block_theme_files();
 ...
 * @return bool
 */
function the7_update_12_10_0_remove_the7_be_plugin() {
	deactivate_plugins(
		[
			'the7-block-editor/the7-block-editor.php',
		],
		true
	);
 ...
 * @return bool
 */
function the7_update_14_0_2_1_fix_broken_bbe_settings() {
	$option_name = 'better-block-editor__user-defined-responsiveness-breakpoints';
 ...
 * @return bool
 */
function the7_update_14_2_1_bbe_rating_notice() {
	\The7\Admin\BBE_Rating_Banner_Check::maybe_show_banner();
